...
% msgbox('Message b');
% t1 = solve('s2(1,1)*(x(1,1)+m(1,1)*t1)+s2(1,2)*(y(1,1)+n(1,1)*t1)+s2(1,3)*(z(1,1)+p(1,1)*t1)+d2','t1');
% t1 = eval(t1);
...
% t2 = solve('s11(1,1)*(x(2,1)+m(2,1)*t2)+s11(1,2)*(y(2,1)+n(2,1)*t2)+s11(1,3)*(z(2,1)+p(2,1)*t2)+d11','t2');
% t2 = eval(t2);
...
% t3 = solve('m(3,1)*(x21-x(3,1)-m(3,1)*t3)+n(3,1)*(y21-y(3,1)-n(3,1)*t3)+p(3,1)*(z21-z(3,1)-p(3,1)*t3)','t3');
% t3 = eval(t3);
